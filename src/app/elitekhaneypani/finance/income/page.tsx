...
import { useGetWaterIncome } from "../hooks/useFinance";
...
export default function IncomePage() {
    ...
    const { data, isLoading, error } = useGetWaterIncome(query);
    const income = data?.Items ?? [];
    const lastPage = data?.LastPage ?? 1;

    return (
        <div className="p-4 pb-24 space-y-4">
            <MobilePageHeader title="Income" />
            ...
            <div className="relative">
                ...
                <input
                    ...
                    placeholder="Search by source or consumer..."
                            ...
                            <div className="flex items-start justify-between gap-2">
                                <div>
                                    <p className="font-semibold text-gray-900 dark:text-white">
                                        {entry.waterincomeSourceName || entry.consumerName || "Income"}
                                    </p>
                                    <p className="text-xs text-gray-500 dark:text-gray-400">#{entry.voucherNo}</p>
                                </div>
                                <span className="text-sm font-bold text-green-600">{formatCurrency(entry.amount)}</span>
                            </div>
                            {entry.description && (
                                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{entry.description}</p>
                            )}
                            <div className="mt-2 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                                <span>{formatDate(entry.incomeDate)}</span>
