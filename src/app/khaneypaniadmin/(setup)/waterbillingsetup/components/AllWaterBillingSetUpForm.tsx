...
import { useDeleteWaterBillingSetUp, useGetAllWaterBillingSetUp } from '../hooks'
import { WaterBillingSetUpResponse } from '../types/IWaterBillingSetup'
import AddWaterBillingSetUp from '../pages/Add'
import EditWaterBillingSetUp from '../pages/Edit'
...
interface ActionMenuProps {
    WaterBillingSetUp: WaterBillingSetUpResponse;
    // onView: (Invoice: InvoiceResponse) => void;
    onEdit: (WaterBillingSetUp: WaterBillingSetUpResponse) => void;
    ...
    return (
        <div className="flex justify-center">
            ...
            {open && (
                ...
                >
                    <button
                        onClick={() => { onEdit(WaterBillingSetUp); setOpen(false) }}
                    ...
                    <button
                        onClick={() => { onDelete(WaterBillingSetUp.id); setOpen(false) }}
...
const AllWaterBillingSetUpForm = () => {
    ...
    const [selectedWaterBillingSetUp, setSelectedWaterBillingSetUp] = useState<WaterBillingSetUpResponse | null>(null)
    ...
    const [selectedWaterBillingSetUpName, setSelectedWaterBillingSetUpName] = useState<string | null>(
        ""
    );
    ...
    const { data, isLoading, error } = useGetAllWaterBillingSetUp(fullQuery)
    const deleteWaterBillingSetUp = useDeleteWaterBillingSetUp()


    const WaterBillingSetUpDetails = data?.Items ?? [];
    ...
    const handleEditLead = (WaterBillingSetUp: WaterBillingSetUpResponse) => {
        setSelectedWaterBillingSetUp(WaterBillingSetUp)
        setEditModal(true)
    }

    const handleAddWaterBillingSetUp = () => {
        setSelectedWaterBillingSetUp(null)
        setAddModal(true)
    };
    ...
    const handleDelete = async (id: string) => {
        try {
            console.error("Id", id)
            await deleteWaterBillingSetUp.mutateAsync(id)
        } catch (error) {
            console.error(error)
        }
    }
                ...
                <div className="bg-white dark:bg-[#353535] border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-8">
                    <div className="text-center py-16">
                        ...
                        <h3 className="text-xl font-medium text-gray-900 dark:text-white mb-2">
                            Error loading WaterBillingSetUp Status
                        </h3>
    ...
    return (
        <>
            ...
            <div className="p-4 sm:p-6">
                <div className="bg-white dark:bg-[#353535] border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div className="flex w-full justify-between p-3 px-4 pt-4 items-center">
                        <h1 className="text-xl font-semibold dark:text-white">All WaterBillingSetUp</h1>
                        <div className="flex items-center space-x-3">
                            ...
                            <ButtonElement
                                icon={<Plus size={18} />}
                                type="button"
                                text="Add WaterBillingSetUp"
                                onClick={handleAddWaterBillingSetUp}
                        ...
                        <div className="mb-6 mx-4 bg-white dark:bg-[#353535] p-5 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                            ...
                            >
                                ...
                                {/* <div className="flex-1 min-w-[240px]">
                                    <AppCombobox
                                        value={selectedWaterBillingSetUpName}
                                        dropDownWidth="w-full"
                                        dropdownPositionClass="absolute"
                                        label="WaterBillingSetUp Name"
                                        name="fullName"
                                        form={form}
                                        options={data?.Items}
                                        selected={
                                            data?.Items?.find(
                                                (g) => g.fullName === selectedWaterBillingSetUpName
                                            ) || null
                                        }
                                        onSelect={(group) => {
                                            if (group) {
                                                setSelectedWaterBillingSetUpName(group.fullName || null);
                                            } else {
                                                setSelectedWaterBillingSetUpName(null);
                                            }
                                        }}
                    ...
                    <div className="px-4 pb-4">
                        <div className="w-full overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-lg">
                            <table className="min-w-full text-sm">
                                ...
                                <tbody>
                                    {WaterBillingSetUpDetails.length === 0 ? (
                                        <tr>
                                            <td colSpan={9} className="p-4 text-center italic text-gray-500 dark:text-gray-400">
                                                No WaterBillingSetUp found.
                                            </td>
                                        </tr>
                                    ) : (
                                        WaterBillingSetUpDetails.map((WaterBillingSetUp, index) => (
                                            <tr
                                                key={WaterBillingSetUp.id}
            ...
            />
            <EditWaterBillingSetUp
                visible={editModal}
                waterBillingSetUp={selectedWaterBillingSetUp}
                onClose={() => {
                    setEditModal(false)
                    setSelectedWaterBillingSetUp(null)
                }}
            ...
            <DeleteComponents
                ...
                id={deleteId ?? ''}
                title="Delete water tariff plan"
                description="Are you sure you want to delete this water tariff plan? This action cannot be undone."
...
export default AllWaterBillingSetUpForm