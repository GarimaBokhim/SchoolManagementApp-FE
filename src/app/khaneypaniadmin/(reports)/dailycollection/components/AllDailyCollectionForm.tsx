...
import { useGetAllDailyCollection } from '../hooks'
...
const AllDailyCollectionForm = () => {
    ...
    const [DailyCollectionForm, setDailyCollectionForm] = useState(false);
    ...
    const [selectedDailyCollectionName, setSelectedDailyCollectionName] = useState<string | null>(
        ""
    );
    ...
    const { data, isLoading, error } = useGetAllDailyCollection(fullQuery)


    const DailyCollectionDetails = data?.Items ?? [];
    ...
    const handleAddDailyCollection = () => {
        setDailyCollectionForm(false);
        setSelectedId("");
    };
            ...
            <div className="p-4 sm:p-6">
                <div className="bg-white dark:bg-[#353535] border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div className="flex w-full justify-between p-3 px-4 pt-4 items-center">
                        <h1 className="text-xl font-semibold dark:text-white">All DailyCollection</h1>
                        ...
                        <div className="mb-6 mx-4 bg-white dark:bg-[#353535] p-5 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                            ...
                            >
                                ...
                                {/* <div className="flex-1 min-w-[240px]">
                                    <AppCombobox
                                        value={selectedDailyCollectionName}
                                        ...
                                        selected={
                                            data?.Items?.find(
                                                (g) => g.consumerName === selectedDailyCollectionName
                                            ) || null
                                        }
                                        onSelect={(group) => {
                                            if (group) {
                                                setSelectedDailyCollectionName(group.consumerName || null);
                                            } else {
                                                setSelectedDailyCollectionName(null);
                                            }
                                        }}
                            ...
                            <table className="min-w-full text-sm">
                                <thead>
                                    <tr className="bg-gray-50 dark:bg-[#80878c] uppercase font-semibold border-b">
                                        ...
                                        <th className="px-4 py-3 text-left">PaymentNumber</th>
                                        <th className="px-4 py-3 text-left">ConsumerName</th>
                                        <th className="px-4 py-3 text-left">PaymentDate</th>
                                        <th className="px-4 py-3 text-left">ReceiptNumber</th>
                                        ...
                                        <th className="px-4 py-3 text-left">CollectedBy</th>

                                        <th className="px-4 py-3 text-center">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {DailyCollectionDetails.length === 0 ? (
                                        <tr>
                                            <td colSpan={9} className="p-4 text-center italic text-gray-500 dark:text-gray-400">
                                                No DailyCollection found.
                                            </td>
                                        </tr>
                                    ) : (
                                        DailyCollectionDetails.map((DailyCollection, index) => (
                                            <tr
                                                key={DailyCollection.PaymentId}
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {DailyCollection.PaymentNumber}
                                                </td>
                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {DailyCollection.ConsumerName}
                                                </td>
                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {DailyCollection.PaymentDate}
                                                </td>
                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {DailyCollection.ReceiptNumber}
                                                </td>

                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {DailyCollection.MeterNumber}
                                                </td>

                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {DailyCollection.PaidAmount}
                                                </td>

                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {DailyCollection.PaymentMethods === 1 && "Cash"}
                                                </td>

                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {DailyCollection.CollectedBy}
                                                </td>
...
export default AllDailyCollectionForm