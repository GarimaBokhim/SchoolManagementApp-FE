    ...
    return (
        <div className="flex justify-center">
            ...
            {open && (
                ...
                >
                    <button
                        onClick={() => { onEdit(WaterReceipts); setOpen(false) }}
                    ...
                    <button
                        onClick={() => { onDelete(WaterReceipts.id); setOpen(false) }}
...
const AllWaterReceiptsForm = () => {
    ...
    const [selectedWaterReceiptsName, setSelectedWaterReceiptsName] = useState<string | null>(
        ""
    );
    ...
    const { data, isLoading, error } = useGetAllWaterReceipt(fullQuery)
    const deleteWaterReceipts = useDeleteWaterReceipt()


    const WaterReceiptsDetails = data?.Items ?? [];
    ...
    const handleDelete = async (id: string) => {
        try {
            console.error("Id", id)
            await deleteWaterReceipts.mutateAsync(id)
        } catch (error) {
            console.error(error)
        }
    }
                ...
                <div className="bg-white dark:bg-[#353535] border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-8">
                    <div className="text-center py-16">
                        ...
                        <h3 className="text-xl font-medium text-gray-900 dark:text-white mb-2">
                            Error loading WaterReceipts Status
                        </h3>
            ...
            <div className="p-4 sm:p-6">
                <div className="bg-white dark:bg-[#353535] border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div className="flex w-full justify-between p-3 px-4 pt-4 items-center">
                        <h1 className="text-xl font-semibold dark:text-white">All WaterReceipts</h1>
                        ...
                        <div className="mb-6 mx-4 bg-white dark:bg-[#353535] p-5 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                            ...
                            >
                                ...
                                {/* <div className="flex-1 min-w-[240px]">
                                    <AppCombobox
                                        value={selectedWaterReceiptsName}
                                        dropDownWidth="w-full"
                                        dropdownPositionClass="absolute"
                                        label="WaterReceipts Name"
                                        name="fullName"
                                        form={form}
                                        options={data?.Items}
                                        selected={
                                            data?.Items?.find(
                                                (g) => g.fullName === selectedWaterReceiptsName
                                            ) || null
                                        }
                                        onSelect={(group) => {
                                            if (group) {
                                                setSelectedWaterReceiptsName(group.fullName || null);
                                            } else {
                                                setSelectedWaterReceiptsName(null);
                                            }
                                        }}
                    ...
                    <div className="px-4 pb-4">
                        <div className="w-full overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-lg">
                            <table className="min-w-full text-sm">
                                ...
                                <tbody>
                                    {WaterReceiptsDetails.length === 0 ? (
                                        <tr>
                                            <td colSpan={9} className="p-4 text-center italic text-gray-500 dark:text-gray-400">
                                                No WaterReceipts found.
                                            </td>
                                        </tr>
                                    ) : (
                                        WaterReceiptsDetails.map((WaterReceipts, index) => (
                                            <tr
                                                key={WaterReceipts.id}
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {WaterReceipts.receiptDate}
                                                </td>

                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {
                                                        paymentMethodsType.find(
                                                            (s) => s.id === Number(WaterReceipts.paymentMethods)
                                                        )?.name
                                                    }
                                                </td>
...
export default AllWaterReceiptsForm