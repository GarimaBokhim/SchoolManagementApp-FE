...
import { useDeleteWaterIncome, useGetAllWaterIncome } from '../hooks'
import { WaterIncomeResponse } from '../types/IWaterIncome'
import AddWaterIncome from '../pages/Add'
import EditWaterIncome from '../pages/Edit'
...
interface ActionMenuProps {
    WaterIncome: WaterIncomeResponse;
    // onView: (Invoice: InvoiceResponse) => void;
    onEdit: (WaterIncome: WaterIncomeResponse) => void;
    ...
    return (
        <div className="flex justify-center">
            ...
            {open && (
                ...
                >
                    <button
                        onClick={() => { onEdit(WaterIncome); setOpen(false) }}
                    ...
                    <button
                        onClick={() => { onDelete(WaterIncome.id); setOpen(false) }}
...
const AllWaterIncomeForm = () => {
    ...
    const [selectedWaterIncome, setSelectedWaterIncome] = useState<WaterIncomeResponse | null>(null)
    ...
    const { data, isLoading, error } = useGetAllWaterIncome(fullQuery)
    const deleteWaterIncome = useDeleteWaterIncome()


    const WaterIncomeDetails = data?.Items ?? [];
    ...
    const handleEditLead = (WaterIncome: WaterIncomeResponse) => {
        setSelectedWaterIncome(WaterIncome)
        setEditModal(true)
    }

    const handleAddWaterIncome = () => {
        setSelectedWaterIncome(null)
        setAddModal(true)
    };
    ...
    const handleDelete = async (id: string) => {
        try {
            console.error("Id", id)
            await deleteWaterIncome.mutateAsync(id)
        } catch (error) {
            console.error(error)
        }
    }
                ...
                <div className="bg-white dark:bg-[#353535] border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-8">
                    <div className="text-center py-16">
                        ...
                        <h3 className="text-xl font-medium text-gray-900 dark:text-white mb-2">
                            Error loading WaterIncome Status
                        </h3>
    ...
    return (
        <>
            ...
            <div className="p-4 sm:p-6">
                <div className="bg-white dark:bg-[#353535] border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div className="flex w-full justify-between p-3 px-4 pt-4 items-center">
                        <h1 className="text-xl font-semibold dark:text-white">All WaterIncome</h1>
                        <div className="flex items-center space-x-3">
                            ...
                            <ButtonElement
                                icon={<Plus size={18} />}
                                type="button"
                                text="Add WaterIncome"
                                onClick={handleAddWaterIncome}
                        ...
                        <div className="mb-6 mx-4 bg-white dark:bg-[#353535] p-5 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                            ...
                            >
                                ...
                                {/* <div className="flex-1 min-w-[240px]">
                                    <AppCombobox
                                        value={selectedWaterIncomeName}
                                        dropDownWidth="w-full"
                                        dropdownPositionClass="absolute"
                                        label="WaterIncome Name"
                                        name="fullName"
                                        form={form}
                                        options={data?.Items}
                                        selected={
                                            data?.Items?.find(
                                                (g) => g.fullName === selectedWaterIncomeName
                                            ) || null
                                        }
                                        onSelect={(group) => {
                                            if (group) {
                                                setSelectedWaterIncomeName(group.fullName || null);
                                            } else {
                                                setSelectedWaterIncomeName(null);
                                            }
                                        }}
                            ...
                            <table className="min-w-full text-sm">
                                <thead>
                                    <tr className="bg-gray-50 dark:bg-[#80878c] uppercase font-semibold border-b">
                                        ...
                                        <th className="px-4 py-3 text-left">voucherNo</th>
                                ...
                                <tbody>
                                    {WaterIncomeDetails.length === 0 ? (
                                        <tr>
                                            <td colSpan={9} className="p-4 text-center italic text-gray-500 dark:text-gray-400">
                                                No WaterIncome found.
                                            </td>
                                        </tr>
                                    ) : (
                                        WaterIncomeDetails.map((WaterIncome, index) => (
                                            <tr
                                                key={WaterIncome.id}
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {WaterIncome.voucherNo}
                                                </td>
                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {WaterIncome.waterincomeSourceName}
                                                </td>
                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {WaterIncome.amount}
                                                </td>
                                                ...
                                                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                                                    {
                                                        paymentMethodsType.find(
                                                            (s) => s.id === Number(WaterIncome.paymentMethods)
                                                        )?.name
                                                    }
                                                </td>
            ...
            <EditWaterIncome
                visible={editModal}
                waterIncome={selectedWaterIncome}
                onClose={() => {
                    setEditModal(false)
                    setSelectedWaterIncome(null)
                }}
            ...
            <DeleteComponents
                ...
                id={deleteId ?? ''}
                title="Delete water income"
                description="Are you sure you want to delete this water income record? This action cannot be undone."
...
export default AllWaterIncomeForm