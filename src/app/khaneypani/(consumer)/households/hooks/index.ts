...
import { AddHouseHoldsPayload, AddHouseHoldsResponse, HouseHoldsResponse, UpdateHouseHoldsPayload } from '../types/IHouseHolds'
...
export const HouseHoldsEndpoints = {
  ...
  update: '/api/KhaneyPaniHouseHolds/UpdateInvoice',
  delete: '/api/KhaneyPaniHouseHolds/DeleteInvoice'
}
...
const normalizeUpdateHouseHoldsPayload = (data: UpdateHouseHoldsPayload): UpdateHouseHoldsPayload => ({
    id:String(data.id ?? '').trim(),
  consumerId: String(data.consumerId ?? '').trim(),
 consumerName: String(data.consumerName ?? '').trim(),
  meterNumber: String(data.meterNumber ?? '').trim(),
...
const normalizeHouseHoldsPayload = (data: AddHouseHoldsPayload): AddHouseHoldsPayload => ({
  consumerId: String(data.consumerId ?? '').trim(),
 consumerName: String(data.consumerName ?? '').trim(),
  meterNumber: String(data.meterNumber ?? '').trim(),
  contactNumber: String(data.contactNumber ?? '').trim(),
