...
import { useEditInstallmentPayments, useGetInstallmentPaymentsById } from "../hooks";
...
type Props = {
    form: UseFormReturn<UpdateInstallmentPaymentsPayload>;
    onClose: () => void;
    onSuccess?: () => void;
    PaymentsId: string;
...
const EditInstallmentPaymentsForm = ({
    form,
    onClose,
    onSuccess,
    PaymentsId,
    ...
    const editInstallmentPayments = useEditInstallmentPayments();

    const { handleError, clearError } = useErrorHandler();
    const { isPrimaryBS } = useDate();

    const { data: InstallmentPaymentsData } = useGetInstallmentPaymentsById(PaymentsId);
    ...
    // =========================
    useEffect(() => {
        if (!InstallmentPaymentsData) return;

        const invoiceId = InstallmentPaymentsData?.invoiceId ?? "";
        const method = Number(InstallmentPaymentsData?.paymentMethod ?? 0);

        let rawDate = InstallmentPaymentsData?.paymentDate ?? "";
        ...
        form.reset({
            invoiceId,
            amount: Number(InstallmentPaymentsData?.amount ?? 0),
    ...
    }, [InstallmentPaymentsData, form]);

    const onSubmit: SubmitHandler<UpdateInstallmentPaymentsPayload> = async (data) => {
        clearError();
        ...
        try {
            ...
            await editInstallmentPayments.mutateAsync({
                id: PaymentsId,
                data: {
                ...
                <fieldset>
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)}>
                        ...
                        <div className="flex justify-center mt-6">
                            <ButtonElement
                                type="submit"
                                text={editInstallmentPayments.isPending ? "Updating..." : "Update Payments"}
                            />
                        </div>
                    </form>
                </fieldset>
...
export default EditInstallmentPaymentsForm;