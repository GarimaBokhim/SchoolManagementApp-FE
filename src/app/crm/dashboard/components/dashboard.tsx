...
import { useDashboardStats } from '../data/mock_data'
...
const Dashboard: React.FC = () => {
  ...
  // API stats — runs independently of auth check
  const { stats, isLoading: statsLoading } = useDashboardStats()
  const normalizedStats = stats.map((stat) => ({
    ...stat,
    count:
      typeof stat.count === 'number' || typeof stat.count === 'string'
        ? stat.count
        : Number(stat.count),
  }))
  const firstRowStats = normalizedStats.slice(0, 5)
  const secondRowStats = normalizedStats.slice(5, 10)
