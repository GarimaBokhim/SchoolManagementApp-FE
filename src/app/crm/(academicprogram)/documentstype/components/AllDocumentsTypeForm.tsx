...
import { useDeleteDocumentsType, useGetAllDocumentsType } from '../hooks'
import { DocumentsTypeResponse } from '../types/IDocumentsType'
...
import EditDocumentsType from '../pages/Edit'
import AddDocumentsType from '../pages/Add'
...
interface ActionMenuProps {
    DocumentsType: DocumentsTypeResponse;
    onEdit: (DocumentsType: DocumentsTypeResponse) => void;
...
const ActionMenu = ({ DocumentsType, onEdit, onDelete, canEdit = true, canDelete = true }: ActionMenuProps) => {
        ...
        <div className="flex justify-center">
            ...
            {open && (
                ...
                >
                    ...
                    {canEdit && <button
                        onClick={() => { onEdit(DocumentsType); setOpen(false) }}
                    ...
                    {canDelete && <button
                        onClick={() => { onDelete(DocumentsType.id); setOpen(false) }}
...
const AllDocumentsTypeForm = () => {
    ...
    const [editDocumentsTypeId, setEditDocumentsTypeId] = useState<string | null>(null)

    const [showDeleteModal, setShowDeleteModal] = useState(false)
    const [deleteDocumentsTypeId, setDeleteDocumentsTypeId] = useState<string | null>(null)
    ...
    const { data, isLoading, error } = useGetAllDocumentsType(params)
    const deleteDocumentsType = useDeleteDocumentsType()
    const DocumentsTypeDetails = data?.items ?? [];
    ...
    const handleEditDocumentsType = (DocumentsType: DocumentsTypeResponse) => {
        setEditDocumentsTypeId(DocumentsType.id)
        setShowEditModal(true)
    }

    const handleDeleteDocumentsType = (id: string) => {
        setDeleteDocumentsTypeId(id)
    ...
    const onDelete = async (id: string) => {
        try {
            await deleteDocumentsType.mutateAsync(id)
        } catch (error) {
            console.error(error)
        }
    }
    ...
    return (
        <>
            ...
            <div className="p-4 sm:p-6">
                    ...
                    <div className="px-4 pb-4">
                        <div className="w-full overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-lg">
                            <table className="min-w-full text-sm">
                                ...
                                <tbody>
                                    {DocumentsTypeDetails.length === 0 ? (
                                        <tr>
                                            <td
                                                colSpan={8}
                                                className="p-4 text-center italic text-gray-500 dark:text-gray-400"
                                            >
                                                No DocumentsType found.
                                            </td>
                                        </tr>
                                    ) : (
                                        DocumentsTypeDetails.map((DocumentsType, index) => (
                                            <tr
                                                key={DocumentsType.id}
                                                ...
                                                <td className="px-4 py-3">
                                                    <span className='flex justify-center gap-3'>
                                                        ...
                                                        <ActionMenu
                                                            ...
                                                            onEdit={handleEditDocumentsType}
                                                            onDelete={handleDeleteDocumentsType}
                                                            // onView={handleView}
                                                            canEdit={true}
                                                            canDelete={true}
                                                        />
                ...
                {/* Pagination */}
                {DocumentsTypeDetails.length > 0 && totalPages > 1 && (
                    <div className="mt-4">
                        <Pagination
                            form={paginationForm}
                            pagination={{
            ...
            {showDeleteModal && deleteDocumentsTypeId && (

                <DeleteComponents
                    visible={showDeleteModal}
                    onClose={() => setShowDeleteModal(false)}
                    onConfirm={onDelete}
                    id={deleteDocumentsTypeId}
                    title="Delete DocumentsType"
                    description="Are you sure you want to delete this DocumentsType?"
                />

            )}

            {showEditModal && editDocumentsTypeId && (

                <EditDocumentsType
                    DocumentsTypeId={editDocumentsTypeId}
...
export default AllDocumentsTypeForm