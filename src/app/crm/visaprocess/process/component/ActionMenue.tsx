...
interface VisaApplicationActionMenuProps {
    visaApplication: IVisaApplication;
    onView: (visaApplication: IVisaApplication) => void;
    onEdit: (visaApplication: IVisaApplication) => void;
    onDelete: (id: string) => void;
    canEdit?: boolean;
    canDelete?: boolean;
}

export const VisaApplicationActionMenu = ({
    visaApplication,
    onView,
    onEdit,
    onDelete,
    canEdit = true,
    canDelete = true,
}: VisaApplicationActionMenuProps) => {
    ...
    return (
        <div className="flex justify-center">
            ...
            {open && (
                ...
                >
                    <button
                        onClick={() => { onView(visaApplication); setOpen(false); }}
                    ...
                    {canEdit && (
                        <button
                            onClick={() => { onEdit(visaApplication); setOpen(false); }}
                            className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors flex items-center gap-2"
                        >
                            <Edit size={14} /> Edit
                        </button>
                    ...
                    {canDelete && (
                        <button
                            onClick={() => { onDelete(visaApplication.id); setOpen(false); }}
