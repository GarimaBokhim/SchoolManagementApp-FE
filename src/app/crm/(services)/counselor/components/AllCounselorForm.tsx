...
import { useDeleteCounselor, useGetAllCounselor } from '../hooks'
import { CounselorResponse } from '../types/ICounselor'
...
import EditCounselor from '../pages/Edit'
import AddCounselor from '../pages/Add'
...
interface ActionMenuProps {
    ...
    onEdit: (Counselor: CounselorResponse) => void;
...
const ActionMenu = ({ Counselor, onEdit, onDelete, canEdit = true, canDelete = true }: ActionMenuProps) => {
        ...
        <div className="flex justify-center">
            ...
            {open && (
                ...
                >
                    ...
                    {canEdit && <button
                        onClick={() => { onEdit(Counselor); setOpen(false) }}
                    ...
                    {canDelete && <button
                        onClick={() => { onDelete(Counselor.id); setOpen(false) }}
...
const AllCounselorForm = () => {
    ...
    const [selectedCouncelorId, setSelectedCounselorId] = useState<string | null>(null)


    const [showEditModal, setShowEditModal] = useState(false)
    const [editCounselorId, setEditCounselorId] = useState<string | null>(null)
    ...
    const { data, isLoading, error } = useGetAllCounselor(params)
    const deleteCounselor = useDeleteCounselor()
    const counselorDetails = data?.items ?? [];
    ...
    const handleEditCounselor = (Counselor: CounselorResponse) => {
        setEditCounselorId(Counselor.id)
        setShowEditModal(true)
    }

    const handleDeleteCounselor = (id: string) => {
        setDeleteCounselorId(id)
    ...
    const onDelete = async (id: string) => {
        try {
            await deleteCounselor.mutateAsync(id)
        } catch (error) {
            console.error(error)
        }
    }
    ...
    return (
        <>
            ...
            <div className="p-4 sm:p-6">
                            ...
                            <table className="min-w-full text-sm">
                                <thead>
                                    <tr className="bg-gray-50 dark:bg-[#80878c] uppercase font-semibold border-b">
                                        ...
                                        <th className="px-4 py-3 text-left hidden md:table-cell">Contact Number</th>

                                        <th className="px-4 py-3 text-center">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {counselorDetails.length === 0 ? (
                                        <tr>
                                            <td
                                                colSpan={8}
                                        ...
                                        counselorDetails.map((counselor, index) => (
                                            <tr
                                                key={counselor.id}
                ...
                {/* Pagination */}
                {counselorDetails.length > 0 && totalPages > 1 && (
                    <div className="mt-4">
                        <Pagination
                            form={paginationForm}
                            pagination={{
            ...
            {showDeleteModal && deleteCounselorId && (

                <DeleteComponents
                    visible={showDeleteModal}
                    onClose={() => setShowDeleteModal(false)}
                    onConfirm={onDelete}
                    id={deleteCounselorId}
...
export default AllCounselorForm