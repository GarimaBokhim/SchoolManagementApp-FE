'use client'

import { useState } from 'react'
import AllClass from './All'
import AllTrainingRegistration from '../../trainingregistration/pages/All'
const AllClassDetails = () => {
    const tabs = [
        { id: 'classes', label: 'Classes' },
        { id: 'trainingRegistration', label: 'TrainingRegistration' }
    ]

    const [activeTab, setActiveTab] = useState<string>('classes')

    const renderContent = () => {
        switch (activeTab) {
            case 'classes':
                return <AllClass />
            case 'trainingRegistration':
                return <AllTrainingRegistration />
            default:
                return <AllClass />
        }
    }

    return (
        <div className="p-4 h-full">
            {/* Tabs */}
            <div className="bg-blue-100 rounded-t-xl px-4 pt-4 flex gap-1">
                {tabs.map((t) => {
                    const isActive = activeTab === t.id
                    return (
                        <button
                            key={t.id}
                            onClick={() => setActiveTab(t.id)}
                            className={
                                'px-6 py-2 text-sm font-medium transition-all ' +
                                (isActive
                                    ? 'text-blue-700 border-b-2 border-blue-700 font-semibold'
                                    : 'text-blue-600 hover:bg-blue-200 rounded-sm')
                            }
                        >
                            {t.label}
                        </button>
                    )
                })}
            </div>

            {/* Content */}
            <div className="border border-gray-200 dark:border-gray-700 rounded-b-lg h-[90%] p-6 bg-white dark:bg-gray-800 transition-all overflow-auto">
                {renderContent()}
            </div>
        </div>
    )
}

export default AllClassDetails