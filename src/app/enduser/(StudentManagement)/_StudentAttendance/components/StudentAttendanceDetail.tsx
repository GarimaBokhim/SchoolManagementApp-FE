...
const MonthlyAttendanceSheet = ({
  ...
  const [selectedAcademicTeamId, setSelectedAcademicTeamId] = useState<string|null>(null)
  ...
  const onSubmit: SubmitHandler<IFilterAttendance> = async (
    formData
  ) => {
    clearError()
    try {
      const queryParams = [
        ...
        formData.academicTeamId
          ...
          : null,
        selectedYear
          ? `yearName=${encodeURIComponent(selectedYear)}`
          : null,
      ...
      <div className="bg-white dark:bg-[#2f2f2f] w-[96%] max-w-7xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl">
        ...
        <div className="overflow-auto px-4 pb-4">
          <table className="w-full border-collapse text-xs">
           ...
           <tbody>
  {attendanceReport?.Students&& attendanceReport?.Students.length >0 ?(attendanceReport?.Students.map((student, index) => (
    <tr
    ...
    >
      ...
      <td className="border px-2 py-2">
        ...
        {allStudents?.Items.find((i)=>i.id === student.StudentId)?.firstName ?? "—"}
      </td>
      {days.map((day) => {
