...
const ActivityEndPoints = {
  ...
  getActivityById: '/api/CocurricularActivities/ActivityById',
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<
        IPaginationResponse<{ id: string; name: string }>
...
export const useGetActivityById = (activityId: string) => {
  return useQuery({
    ...
    queryFn: async (): Promise<Activity> => {
      if (!activityId) {
        throw new Error('Id is required to get a Activity')
      }
  ...
  return useMutation<Participation, Error, AddParticipationPayload>({
    mutationFn: async (payload) => {
      const response = await api.post(
        ActivityEndPoints.addParticipation,
        payload
      )
      return response.data
    },
...
export const useGetAllEvents = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<
        IPaginationResponse<{ id: string; title: string }>
...
export const useDeleteActivity = () => {
  ...
  return useMutation<void, Error, string>({
    mutationFn: async (id) => {
      await api.delete(`/api/CocurricularActivities/DeleteActivity/${id}`)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [activityQueryKey] })
    },
