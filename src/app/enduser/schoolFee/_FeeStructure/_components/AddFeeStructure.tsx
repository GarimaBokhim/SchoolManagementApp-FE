...
const AddFeeStructureForm = ({ form, onClose }: Props) => {
  ...
  const [selectedPaidTypes, setSelectedPaidTypes] = useState(0)
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-start">
              ...
              <AppCombobox
                ...
                form={form}
                required
                options={FEE_PAID_TYPE_OPTIONS}
                selected={
                  FEE_PAID_TYPE_OPTIONS?.find(
                    (fc) => fc.value === selectedPaidTypes
                  ) || null
                }
                onSelect={(fc) => setSelectedPaidTypes(fc?.value ?? 0)}
                getLabel={(fc) => fc?.label ?? ''}
                getValue={(fc) => fc?.value ?? ''}
              />
            </div>
            ...
            <div className="flex justify-center mt-8">
              <ButtonElement
                type="submit"
                text="Submit"
                className="bg-teal-500 cursor-pointer  hover:bg-teal-600 text-white px-6 py-2 rounded-lg shadow-md transition-all"
              />
            </div>
          </form>
        </fieldset>
