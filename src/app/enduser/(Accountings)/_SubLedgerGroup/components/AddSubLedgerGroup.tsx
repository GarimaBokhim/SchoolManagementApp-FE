...
import { useAddSubLedgerGroup } from "../hooks";
type Props = {
  form: UseFormReturn<ISubLedgerGroup>;
  onClose: () => void;
};

const AddSubLedgerGroupForm = ({ form, onClose }: Props) => {
  const addSubLedgerGroup = useAddSubLedgerGroup();
  ...
  const onSubmit: SubmitHandler<ISubLedgerGroup> = async (data) => {
    clearError();
    try {
      await toast.promise(addSubLedgerGroup.mutateAsync(data), {
        loading: "Adding sub ledger group...",
        success: "Successfully added sub ledger group",
      });
      handleClose();
    } catch (error) {
...
export default AddSubLedgerGroupForm;
