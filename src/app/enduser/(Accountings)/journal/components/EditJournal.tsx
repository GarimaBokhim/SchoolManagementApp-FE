...
const EditJournalForm = ({ form, onClose, journalId }: Props) => {
  const editJournal = useEditJournal();
  const { data: journalData } = useGetJournalById(journalId);
  ...
  const onSubmit: SubmitHandler<IJournal> = async (form) => {
    clearError();
    try {
      await toast.promise(
        editJournal.mutateAsync({
          id: journalId,
          data: form,
        }),
        {
  ...
  useEffect(() => {
    if (journalData) {
      ...
      journalData.journalEntries?.forEach((entry, idx) => {
        ledgerMap[idx] = entry.ledgerId;
      });

      setSelectedLedgerId(ledgerMap);
    }
  }, [journalData, form]);
