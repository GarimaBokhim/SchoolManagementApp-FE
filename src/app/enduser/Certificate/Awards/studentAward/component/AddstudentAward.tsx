  ...
  const onSubmit: SubmitHandler<Istudentaward> = async (formData) => {
    try {
      await addAwardMutation.mutateAsync({
        ...
        awardDescriptions: formData.awardDescriptions,
        certificateTemplateId: formData.certificateTemplateId,
        eventsId: formData.eventsId,
        contentHtml: formData.contentHtml,
      });

      toast.success("Student award added successfully!", { duration: 2000 });
      reset();
      ...
      toast.error(error.message || "Failed to add student award.", { duration: 2000 });
    }
  };
      ...
      <div className="bg-white dark:bg-[#353535] w-full h-full p-6 relative">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Award Title — used on certificate if filled */}
            <InputElement
              label="Award Title"
              name="awardTitle"
              inputType="text"
              form={form}
              placeholder="e.g. Best Student Award (overrides template title)"
            />
            ...
            <AppCombobox
              ...
              onSelect={(template) => {
                ...
                setValue("certificateTemplateId", template?.id ?? "");
              }}
