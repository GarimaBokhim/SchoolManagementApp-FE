...
// ── Gender helpers ──────────────────────────────────────────────
const getTitle = (g?: number) => (g === 2 ? "Ms." : "Mr.")
const getRelation = (g?: number) => (g === 2 ? "daughter of Mr." : "son of Mr.")
const getPronounSubject = (g?: number) => (g === 2 ? "She" : g === 1 ? "He" : "They")
const getPronounPossessive = (g?: number) => (g === 2 ? "Her" : g === 1 ? "His" : "Their")

const CollegeCertificate: React.FC<Props> = ({ studentId, onClose, examId }) => {
  ...
  const { data: certificateData } = useGenerateCertificateByStudent(studentId, examId)

  // ── School data ─────────────────────────────────────────────
  const { data: schoolResponse } = useGetAllSchool()
  const school = schoolResponse?.Items?.[0]

  const gender = StudentData?.genderStatus

  const provinceName = allProvince?.Items.find(
    (i) => i.Id === Number(certificateData?.provinceId)
  )?.provinceNameInEnglish

  const districtName = allDistrict?.Items.find(
          ...
          <style>
            @media print {
              ...
              #certificate::before {
                ...
                position: absolute;
                inset: 0;
                background-image: url('${schoolLogoUrl}');
              ...
              #printBtn, #generateBtn { display: none !important; }
              * {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center bg-black/40 backdrop-blur-sm ml-12 md:ml-64 sm:ml-16 xs:ml-0">
      <div className="bg-[#FBFBFB] dark:bg-[#27272a] w-full max-w-[95vw] md:max-w-[85vw] lg:max-w-[75vw] xl:max-w-[70vw] h-full rounded-lg overflow-auto md:p-8 shadow-lg">
        ...
        >
          ...
          <header className="pb-4 mb-2">
            <div className="flex items-start gap-2">
              ...
              <div className="w-28 mt-6">
                <img src={schoolLogoUrl} alt="Logo" className="w-full h-auto" />
              </div>
              ...
              <div className="flex ml-[-12%] w-full">
                <div className="w-full">
                  <div className="grid grid-cols-3 text-sm font-medium">
                    ...
                    {/* Left: registration */}
                    <div className="ml-2">
                      <p>TU Regd. No.: {school?.shortName ?? '-'}</p>
                    </div>
                    ...
                    <div className="text-right text-xs text-gray-700">
                      <p>Phone: {school?.contactNumber ?? '-'}</p>
                      <p>{school?.email ?? '-'}</p>
                    </div>

                  </div>

                  {/* School full name + address */}
                  <div className="text-center">
          ...
          <div className="flex items-start mb-6 justify-between">
            <p className="mt-[1.7rem] font-semibold">
              <strong>S. No.:</strong> {certificateData?.registrationNumber ?? '-'}
            </p>
          ...
          <div className="flex gap-8 mb-6 items-start">
            <div className="flex-1 text-[16px] leading-relaxed">
              <p className="text-justify my-3">
                ...
                secured{" "}
                <strong>{certificateData?.percentage}%</strong> with{" "}
                <strong>{certificateData?.division}</strong> division.{" "}
                {getPronounPossessive(gender)} conduct while at college was
                commendable. As per our record,{" "}
                {getPronounPossessive(gender).toLowerCase()} date of birth is{" "}
              ...
              <p className="italic mt-10 text-start">
                ...
                {getPronounPossessive(gender).toLowerCase()} future endeavors
                and success in life.
              </p>
            </div>
          </div>
