...
const AssignClass = ({ teacherId, visible, onClose }: Props) => {
  ...
  useEffect(() => {
    setSelectedSubjects((prev) =>
      prev.filter((id) => filteredSubjects.some((sub) => sub.Id === id))
    )
  }, [selectedClasses])
  ...
  return (
    <div className=" absolute bg-white dark:bg-[#3a3a3a] p-4 rounded-xl shadow-md border border-gray-200 w-[20rem] md:w-[12rem] sm:w-[16rem] max-h-[30vh] overflow-y-auto z-40 ml-[-20%] ">
      <h1 className="text-md font-semibold mb-3">Assign Class & Subject</h1>
        ...
        <div>
          ...
          <div className=" relative space-y-2">
            {allClass?.Items?.map((cls) => {
              ...
              return (
                <div key={cls.id}>
                  <label className="flex items-center gap-2   rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                    <input
                      type="checkbox"
